...
class QemuEnvBuilder(venv.EnvBuilder):
    ...
    def ensure_directories(self, env_dir: DirType) -> SimpleNamespace:
        ...
        def symlink_or_copy(
            ...
            try:
                orig_symlink(src, dst, target_is_directory=target_is_directory)
            except PermissionError as exc:
                if exc.errno not in (errno.EPERM, errno.EACCES, errno.ENOTSUP):
                    raise

                src_path = os.path.join(os.path.dirname(dst), os.fspath(src))
    ...
    def create(self, env_dir: DirType) -> None:
        ...
        try:
            ...
            if not self.symlinks or exc.errno not in (
                errno.EPERM,
                errno.EACCES,
                errno.ENOTSUP,
            ):
                raise
            logger.warning(
                "symlink-based venv creation failed at '%s'; retrying with copies",
                env_dir,
            )
            self.symlinks = False
...
def make_venv(  # pylint: disable=too-many-arguments
    ...
    try:
        ...
        try:
            ...
            if not builder.symlinks or exc.errno not in (
                errno.EPERM,
                errno.EACCES,
                errno.ENOTSUP,
            ):
